...
public sealed class PolygonFloorPrimitive : IPrimitive
{
    ...
    public ArrayMesh BuildMesh(PrimitiveInstanceData data, BuildContext ctx)
    {
        ...
        if ((railStyle != 0 || holeRailStyle != 0) && pts.Count >= 3)
        {
            ...
            if (railStyle != 0)
                any |= RailBuilder.EmitRing(rail, outer2d, centroid2d, railStyle,
            ...
            if (holeRailStyle != 0 && bridged != null)
            {
                ...
                foreach (List<Vector3> ring in bridged)
                {
                    ...
                    any |= RailBuilder.EmitRing(rail, hole2d, Centroid2d(hole2d), holeRailStyle, hh, hw, ha, inward: false);
                }
            }
    ...
    // --- Auto-rail moved to RailBuilder (shared with the circle / half-circle primitives). The outer outline
    // and each bridged hole call RailBuilder.EmitRing (closed ring); the geometry is unchanged by the move.

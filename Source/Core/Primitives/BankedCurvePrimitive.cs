...
/// centreline starts at the local origin heading along local +X and curves toward −Z (a left turn),
/// sweeping <c>arc</c> degrees at centreline <c>radius</c>, spanning <c>width</c> across the path.
/// The cross-section can be <c>bank</c>ed (outer edge raised, rotated about the path tangent) and is a
/// constant-<c>thickness</c> slab whose underside is offset straight down (−Y) — never self-intersects
/// below a 90° bank, and the thickness variation is invisible.
///
...
/// verified top quad (near-inner, near-outer, far-outer, far-inner from the up-facing normal).
/// Right turns come from a second draw-tool preset that mirrors the same TypeId (see ToolManager).
/// </summary>
public sealed class BankedCurvePrimitive : IPrimitive
{
    ...
    public IReadOnlyList<ParamSpec> Parameters { get; } = new[]
    {
        ...
        new ParamSpec("arc",       "Arc (deg)", ParamType.Float, 90.0f, 5f,    270f),
    ...
    public ArrayMesh BuildMesh(PrimitiveInstanceData data, BuildContext ctx)
    {
        ...
        for (int i = 0; i <= seg; i++)
        {
            ...
            // Centre of curvature at (0,0,-R): centreline P(θ)=(R·sinθ, 0, R·cosθ−R), tangent (cosθ,0,−sinθ).
            var p = new Vector3(radius * s, 0, radius * c - radius);
            var radialOut = new Vector3(s, 0, c);                       // away from centre (toward outer edge)
        ...
        for (int i = 0; i < seg; i++)
        {
            ...
            MeshBuilder.AddQuad(surface, innerTop[i], outerTop[i], outerTop[j], innerTop[j], n,
                new Vector2(arcU[i], 0), new Vector2(arcU[i], width),
                new Vector2(arcU[j], width), new Vector2(arcU[j], 0));

            // Underside (faces roughly down).
            MeshBuilder.AddQuad(side, outerBot[i], innerBot[i], innerBot[j], outerBot[j], -n);

            // Outer wall (faces outward); inner wall (faces toward the centre).
            MeshBuilder.AddQuad(side, outerBot[i], outerBot[j], outerTop[j], outerTop[i],
                Radial(i, j, +1));
            MeshBuilder.AddQuad(side, innerBot[j], innerBot[i], innerTop[i], innerTop[j],
                Radial(i, j, -1));
        }

        // End caps: the cross-section quad at each end, facing along ∓ the path tangent.
        var tan0 = new Vector3(1, 0, 0);
        var tanN = new Vector3(Mathf.Cos(arc), 0, -Mathf.Sin(arc));
        MeshBuilder.AddQuad(side, innerBot[0], outerBot[0], outerTop[0], innerTop[0], -tan0);
        MeshBuilder.AddQuad(side, outerBot[seg], innerBot[seg], innerTop[seg], outerTop[seg], tanN);

...
**A parameter value can be variable-length geometry, not just a scalar.** `path_sweep` (see `PRIMITIVES.md`) stores its control points as a `Godot.Collections.Array<Vector3>` under `Parameters["points"]` and a parallel per-point bank `Array<float>` under `Parameters["banks"]`. These are Variant-native typed arrays, so they serialize inline and round-trip like any other param (verified on disk — points + banks survive save→reopen, aligned, across insert/remove). Two rules keep them safe: (1) read them back element-wise (`AsGodotArray()` then `AsVector3()`/`AsSingle()`), not via a typed-array cast, to avoid the silent-empty trap below; (2) any pair of arrays that must stay index-aligned (points↔banks) is edited and snapshotted **together** in one command (`EditPathCommand`) so undo/redo can't drift them apart. A param with no `ParamSpec` (like `points`/`banks`) simply isn't shown by the inspector, which iterates the primitive's declared specs.
